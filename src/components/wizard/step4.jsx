...
const Step4 = function (props) {
  ...
  const handleTechItemClick = (tech) => {
    setTechs(techs.filter((p) => p._id !== tech._id));
    setSelectedTechs([...selectedTechs, tech]);
  };

  const handleSelectedItemClick = (tech) => {
    setSelectedTechs(selectedTechs.filter((p) => p._id !== tech._id));
    setTechs([...techs, tech]);
  };
      ...
      <form onSubmit={handleSubmit(handleSaveProject)}>
        <div className="form-information">
          ...
          <input
            ...
            defaultValue={selectedTechs.map((p) => p._id)}
          />
        </div>
            ...
            <div className="arrowBox">
              {currentStep > 1 ? (
                <FaArrowCircleLeft
                  ...
                  onClick={() => {
                    handleSaveStep({
                      technologies: selectedTechs.map((p) => p._id),
                    });
                    previousStep();
                  }}
                />
            ...
            <div className="arrowBox">
              {currentStep < totalSteps ? (
                <FaArrowCircleRight
                  className="nav-buttons"
                  onClick={() => {
                    handleSaveStep(selectedTechs.map((p) => p._id));
                    nextStep();
                  }}
                />
              ) : (
