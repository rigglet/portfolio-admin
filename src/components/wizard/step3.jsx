...
const Step3 = function (props) {
  ...
  const handleSelectedItemClick = (pack) => {
    setSelectedPacks(selectedPacks.filter((p) => p._id !== pack._id));
    setPacks([...packs, pack]);
  };
  ...
  return (
    <StyledStep>
      ...
      <div className="form-information">
        ...
        <div className="list">
          {packs
            .filter((p) => !selectedPacks.includes(p))
            .map((p) => (
        ...
        <form>
          <input
            ...
            //autoComplete="off"
            //size="40"
            defaultValue={selectedPacks.map((p) => p._id)}
          />
          {/*
          <h4>Technologies:</h4>
          ...
          <div className="arrowBox">
            {currentStep > 1 ? (
              <FaArrowCircleLeft
                ...
                onClick={() => {
                  handleSaveStep({ packages: selectedPacks.map((p) => p._id) });
                  previousStep();
                }}
              />
            ) : (
          ...
          <div className="arrowBox">
            {currentStep < totalSteps ? (
              <FaArrowCircleRight
                className="nav-buttons"
                onClick={() => {
                  handleSaveStep(selectedPacks.map((p) => p._id));
                  nextStep();
                }}
              />
            ) : (
